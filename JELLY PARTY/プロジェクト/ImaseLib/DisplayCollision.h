...
namespace Imase
{
	...
	private:
		...
		struct Mesh
		{
			const std::vector<DirectX::SimpleMath::Vector3>& vertexes;	// 頂点バッファ
			const std::vector<uint32_t>& indexes;						// インデックスバッファ
			DirectX::SimpleMath::Vector3 position;						// 移動
			DirectX::SimpleMath::Quaternion rotate;						// 回転
			DirectX::SimpleMath::Color lineColor;						// 色（ライン用）

			constexpr Mesh(
				const std::vector<DirectX::SimpleMath::Vector3>& vertexes,
		...
		// プリミティブバッチ（メッシュ用）
		std::unique_ptr<DirectX::PrimitiveBatch<DirectX::VertexPositionNormalColor>> m_meshBatch;
