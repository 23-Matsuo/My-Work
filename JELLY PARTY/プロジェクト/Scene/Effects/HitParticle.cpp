...
// インプットレイアウト
//-----------------------------------
const std::vector<D3D11_INPUT_ELEMENT_DESC> HitParticle::INPUT_LAYOUT =
{
...
// 生成関数
//-----------------------------------
void HitParticle::Create(DX::DeviceResources* pDR)
{	
	m_pDR = pDR;
...
// パーティクル作成
//-----------------------------------
void HitParticle::CreateHitParticle(
...
)
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		ParticleUtility pU(
			0.5f,															// 生存時間
			position,														// 基準座標
			DirectX::SimpleMath::Vector3(x * 2, z * 2, -1.0f),				// 速度
			DirectX::SimpleMath::Vector3(-x, -z, 1.0f),						// 加速度
			DirectX::SimpleMath::Vector3(0.2f, 0.2f, 1.0f),					// 初期スケール
			DirectX::SimpleMath::Vector3(0.0f, 0.0f, 1.0f),					// 最終スケール
			DirectX::SimpleMath::Color(color.x, color.y, color.z, 0.8f),	// 初期カラー
			DirectX::SimpleMath::Color(color.x, color.y, color.z, 0.8f)		// 最終カラー
		);
...
void HitParticle::Render()
{
	...
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> texture = 
		Resources::GetInstance()->GetTexture(L"Particle").texture;
...
/// ビルボード作成関数
//-----------------------------------
void HitParticle::CreateBillboard(
