...
// インプットレイアウト
//-----------------------------------
const std::vector<D3D11_INPUT_ELEMENT_DESC> LandParticle::INPUT_LAYOUT =
{
...
// 生成関数
//-----------------------------------
void LandParticle::Create(DX::DeviceResources* pDR)
{	
	m_pDR = pDR;
...
void LandParticle::CreateLandParticle(DirectX::SimpleMath::Vector3 position)
{
		ParticleUtility pU(
			0.3f,													// 生存時間
			position + DirectX::SimpleMath::Vector3(0, 0.3f, 0),	// 基準座標
			DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f),			// 速度
			DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f),			// 加速度
			DirectX::SimpleMath::Vector3(1.5f, 0.5f, 1.0f),			// 初期スケール
			DirectX::SimpleMath::Vector3(2.0f, 1.0f, 1.0f),			// 最終スケール
			DirectX::SimpleMath::Color(1.f, 1.f, 1.0f, 0.5f),		// 初期カラー
			DirectX::SimpleMath::Color(1.f, 1.f, 1.0f, 0.0f)		// 最終カラー
		);
...
// 更新
//-----------------------------------
void LandParticle::Update(const DX::StepTimer& timer)
{
...
// Shader作成
//-----------------------------------
void LandParticle::CreateShader()
{
...
void LandParticle::Render()
{
	...
	for (auto& pU : m_particleUtility)
	{
		...
		vPCT.position = (pU.GetPosition());
		vPCT.color = pU.GetNowColor();
		vPCT.textureCoordinate = { pU.GetNowScale().x, pU.GetNowScale().y };
	...
	// 板ポリゴンを描画
	batch->Begin();
	batch->Draw(D3D11_PRIMITIVE_TOPOLOGY_POINTLIST, &m_vertices[0], 1);
	batch->End();
...
/// ビルボード作成関数
//-----------------------------------
void LandParticle::CreateBillboard(
