...
// インプットレイアウト
//-----------------------------------
const std::vector<D3D11_INPUT_ELEMENT_DESC> JumpParticle::INPUT_LAYOUT =
{
...
// 生成関数
//-----------------------------------
void JumpParticle::Create(DX::DeviceResources* pDR)
{	
	m_pDR = pDR;
...
void JumpParticle::CreateJumpParticle(DirectX::SimpleMath::Vector3 position)
{
	...
	for (int i = 0; i < PARTICLE_NUM; i++)
	{
		...
		float x = RADIUS * std::cos(angle * DirectX::XM_PI / 180.0f) * 2;
		float z = RADIUS * std::sin(angle * DirectX::XM_PI / 180.0f) * 2;
		ParticleUtility pU(
			0.3f,														// 生存時間
			position,													// 基準座標
			DirectX::SimpleMath::Vector3(x, -3.0f, z ),					// 速度
			DirectX::SimpleMath::Vector3(-x, -1.0f, -z),				// 加速度
			DirectX::SimpleMath::Vector3(0.8f, 0.5f, 1.0f),				// 初期スケール
			DirectX::SimpleMath::Vector3(0.2f, 0.2f, 1.0f),				// 最終スケール
			DirectX::SimpleMath::Color(1.f, 1.f, 1.0f, 1.0f),			// 初期カラー
...
// 更新
//-----------------------------------
void JumpParticle::Update(const DX::StepTimer& timer)
{
...
// Shader作成
//-----------------------------------
void JumpParticle::CreateShader()
{
...
/// ビルボード作成関数
//-----------------------------------
void JumpParticle::CreateBillboard(
