...
// インプットレイアウト
//-----------------------------------
const std::vector<D3D11_INPUT_ELEMENT_DESC> RunParticle::INPUT_LAYOUT =
{
...
// 生成関数
//-----------------------------------
void RunParticle::Create(DX::DeviceResources* pDR)
{	
	m_pDR = pDR;
...
void RunParticle::CreateRunParticle(const float& elapsedTime, const DirectX::SimpleMath::Vector3& position)
{
	// 3.0秒ごとに生成
	m_timer += elapsedTime;
	if (m_timer >= 0.15f)
	{
		ParticleUtility pU(
			0.5f,													// 生存時間
			position + DirectX::SimpleMath::Vector3(0, 0.5f,0),		// 基準座標
			DirectX::SimpleMath::Vector3(0.0f, -1.0f, 0.0f),		// 速度
			DirectX::SimpleMath::Vector3(0.0f, -0.5f, 0.0f),		// 加速度
			DirectX::SimpleMath::Vector3(1.2f, 1.0f, 1.0f),			// 初期スケール
			DirectX::SimpleMath::Vector3(0.0f, 0.0f, 1.0f),			// 最終スケール
			DirectX::SimpleMath::Color(0.95f, 0.95f, 0.95f, 1.0f),	// 初期カラー、
			DirectX::SimpleMath::Color(0.95f, 0.95f, 0.95f, 1.0f)	// 最終カラー
		);
...
// 更新
//-----------------------------------
void RunParticle::Update(const DX::StepTimer& timer)
{
...
/// ビルボード作成関数
//-----------------------------------
void RunParticle::CreateBillboard(
